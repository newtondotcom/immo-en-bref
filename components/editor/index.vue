<template>
	<div class="flex w-[40vw] flex-col border-2 bg-neutral-50 px-1 py-1">
		<ToggleGroup type="multiple" v-if="editor">
			...
			<ToggleGroupItem value="undo" aria-label="Undo" @click="editor.chain().focus().undo().run()">
				<Undo class="h-4 w-4" />
			</ToggleGroupItem>

			<ToggleGroupItem value="redo" aria-label="Redo" @click="editor.chain().focus().redo().run()">
				<Redo class="h-4 w-4" />
			</ToggleGroupItem>
		</ToggleGroup>
